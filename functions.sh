...
EOF
           ...
           ssh -t $@ "$RCMD;"' SSHS=true bash -c "function bash() { echo "test" >&2; /bin/bash --rcfile '"${REMOTETMPBASHCONFIG}"' \"\$@\" /dev/null; }; export -f bash; exec bash  --rcfile '"${REMOTETMPBASHCONFIG}"' -i"'
           rm "${TMPBASHCONFIG}"
        else
